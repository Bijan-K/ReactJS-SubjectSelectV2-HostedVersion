...
const Ekhtiyari = (props) => {
  ...
  return (
    <div
      dir="rtl"
      id="omomi"
      className="flex flex-col animate-fadein gap-5 h-screen "
    >
      {/* first table */}
      <div className="bg-slate-800 rounded flex flex-col relative">
        <div
          id="hoverMouse1"
          className="absolute flex flex-col justify-center items-center text-white  border-2 border-gray-900 border-t-4 opacity-0 h-44 w-40 z-30 bg-slate-800 left-0 rounded"
        >
          <HoverC id={hoverV} table={ekhtiyari}></HoverC>
        </div>

        {/* the content */}
        <div className=" text-center rounded-t text-white bg-black p-2">
          در کل 8 واحد، 2 واحد آزمایشگاه
        </div>
        <div className=" grid grid-cols-6 items-center rounded-t text-white bg-black content-center p-3">
          <div className="col-span-5 text-center">درس</div>
          <div className="col-span-1 text-center">واحد</div>
        </div>
        <div
          id="1t"
          className="grid grid-cols-6 content-center text-white gap-2 p-4"
        >
          <button
            onClick={btnClicked}
